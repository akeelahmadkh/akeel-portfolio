...
export const FALLBACK_SKILLS: Skill[] = [
  { _id: 's1', name: 'React Native', category: 'Mobile Development', level: 'Working With' },
  { _id: 's2', name: 'Flutter', category: 'Mobile Development', level: 'Learning' },
  { _id: 's3', name: 'Kotlin', category: 'Mobile Development', level: 'Exploring' },
  { _id: 's4', name: 'Expo CLI', category: 'Mobile Development', level: 'Working With' },
  { _id: 's5', name: 'Python', category: 'AI & Machine Learning', level: 'Working With' },
  { _id: 's6', name: 'NumPy & Pandas', category: 'AI & Machine Learning', level: 'Working With' },
  { _id: 's7', name: 'Machine Learning', category: 'AI & Machine Learning', level: 'Working With' },
  { _id: 's8', name: 'Deep Learning', category: 'AI & Machine Learning', level: 'Learning' },
  { _id: 's9', name: 'RNN & LSTM', category: 'AI & Machine Learning', level: 'Learning' },
  { _id: 's10', name: 'PyTorch / TensorFlow', category: 'AI & Machine Learning', level: 'Learning' },
  { _id: 's11', name: 'Node.js', category: 'Backend & APIs', level: 'Working With' },
  { _id: 's12', name: 'Express.js', category: 'Backend & APIs', level: 'Working With' },
  { _id: 's13', name: 'RESTful APIs', category: 'Backend & APIs', level: 'Working With' },
  { _id: 's14', name: 'Firebase', category: 'Backend & APIs', level: 'Working With' },
  { _id: 's15', name: 'JavaScript (ES6+)', category: 'Programming Languages', level: 'Working With' },
  { _id: 's16', name: 'TypeScript', category: 'Programming Languages', level: 'Working With' },
  { _id: 's17', name: 'Java', category: 'Programming Languages', level: 'Learning' },
  { _id: 's18', name: 'MongoDB & Mongoose', category: 'Databases & Tools', level: 'Working With' },
  { _id: 's19', name: 'Git & GitHub', category: 'Databases & Tools', level: 'Working With' },
];

const fetchWithFallback = async (endpoint: string, options?: any) => {
  try {
    return await axios({ url: `${PRIMARY_API_URL}${endpoint}`, timeout: 2500, ...options });
  } catch (err1) {
    try {
      return await axios({ url: `${FALLBACK_API_URL}${endpoint}`, timeout: 2500, ...options });
    } catch (err2) {
      throw err2;
    }
  }
...
export const adminLogin = async (credentials: { email: string; password: string }) => {
  const res = await fetchWithFallback('/auth/login', { method: 'POST', data: credentials });
  return res.data;
};

export const createProject = async (projectData: Partial<Project>, token: string) => {
  const res = await fetchWithFallback('/projects', {
    method: 'POST',
    data: projectData,
    headers: { Authorization: `Bearer ${token}` },
...
export const updateProject = async (id: string, projectData: Partial<Project>, token: string) => {
...
export const updateSkill = async (id: string, skillData: Partial<Skill>, token: string) => {
...
export const addAdminUser = async (data: { newAdminEmail: string; newAdminPassword: string }, token: string) => {
